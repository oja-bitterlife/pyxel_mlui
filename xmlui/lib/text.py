...
# #############################################################################
# ラベル
class Label(XUElement):
    def __init__(self, state:XUElement, align_attr:str, valign_attr:str):
...
class Msg(XUTextPage):
    ...
    @classmethod
    def clear_msg(cls, state:XUElement, item_tag:str):
        page = XUTextPage(state, item_tag)
        page.clear_pages()

    @classmethod
    def append_msg(cls, state:XUElement, item_tag:str, text:str, all_params:dict[str,Any], page_line_num_attr:str, wrap_attr:str):
        page = XUTextPage(state, item_tag)
        page.append_pages(text, all_params, page_line_num_attr, wrap_attr)

    @classmethod
    def start_msg(cls, state:XUElement, item_tag:str, text:str, all_params:dict[str,Any], page_line_num_attr:str, wrap_attr:str):
        page = XUTextPage(state, item_tag)
...
class MsgDQ(MsgScr):
    ...
    @classmethod
    def start_talk(cls, state:XUElement, item_tag:str, text:str, all_params:dict[str,Any], page_line_num_attr:str, wrap_attr:str):
        state.set_attr(cls.IS_TALK_ATTR, True)
        cls.start_msg(state, item_tag, text, all_params, page_line_num_attr, wrap_attr)

    @classmethod
    def start_system(cls, state:XUElement, item_tag:str, text:str, all_params:dict[str,Any], page_line_num_attr:str, wrap_attr):
        state.set_attr(cls.IS_TALK_ATTR, False)
        cls.start_msg(state, item_tag, text, all_params, page_line_num_attr, wrap_attr)

    @classmethod
    def is_talk(cls, state:XUElement):
