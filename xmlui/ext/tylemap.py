...
class XUXTIlemap:
    ...
    def draw(self, x:int, y:int, tile_no:int, *, rotate:float|None=None, scale:float|None=None):
        tile_num = self.tile_area.w // self.tile_size
        u = self.tile_size * (tile_no % tile_num)
        v = self.tile_size * (tile_no // tile_num)
        pyxel.images[0].blt(x, y, self.image_bank, u, v, self.tile_size, self.tile_size, self.color_key, rotate=rotate, scale=scale)
